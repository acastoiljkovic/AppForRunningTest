...
public class AddKorisnikService extends AsyncTask<String,Void,String> {

    Fragment fragment;

    public AddKorisnikService(Fragment fragment){
    ...
    protected String doInBackground(String... params) {
        try {
            String post_data = URLEncoder.encode("username", "UTF-8") + "=" + URLEncoder.encode(params[0], "UTF-8") + "&" +
                    ...
                    URLEncoder.encode("avatar", "UTF-8") + "=" + URLEncoder.encode(params[9], "UTF-8");
            URL url = new URL(Constants.URL+"/"+Constants.REGISTER);
    ...
    protected void onPostExecute(String s) {
        if(s.equals("Success")){
            ((FragmentRegister)fragment).successRegister();
        }
        else
            ((FragmentRegister)fragment).failedRegister();
    }
